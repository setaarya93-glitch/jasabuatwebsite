...
@section('header', 'Tambah Fitur Kalkulator')
...
<div class="max-w-2xl">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-6 border-b border-gray-100">
            <h3 class="text-lg font-semibold text-gray-800">Form Tambah Fitur</h3>
            <p class="text-sm text-gray-500 mt-1">Tambahkan fitur atau layanan baru ke kalkulator.</p>
        </div>

        <form action="{{ route('dashboard.calculator-features.store') }}" method="POST" class="p-6 space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-1">Nama yang akan ditampilkan di kalkulator.</p>
                </div>
            ...
            <div>
                ...
                <textarea name="description" rows="3"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    placeholder="Deskripsi singkat fitur ini...">{{ old('description') }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="category" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        <option value="feature" {{ old('category') == 'feature' ? 'selected' : '' }}>Fitur Website</option>
                        <option value="service" {{ old('category') == 'service' ? 'selected' : '' }}>Layanan Dukungan</option>
                    </select>
                </div>
            </div>
            ...
            <div class="flex items-center gap-3 pt-4 border-t">
                <button type="submit" class="btn-yellow px-5 py-2 rounded-lg font-medium flex items-center">
                    <i data-lucide="save" class="w-4 h-4 mr-2"></i> Simpan Fitur
                </button>
